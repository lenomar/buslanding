...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		...
		switch (M.provider) {
			...
			case "dfp":
				...
				M.dfp_ad_unit_code = "/" + M.dfp_network_code + "/InGameInterstitial//" + self.config.aid + "//d0";
				M.gam_sizes_mode = 1;
				M.gam_demand_source = 0;

				faZepto.getScript(
					self.debug ? "https://imasdk.googleapis.com/js/sdkloader/ima3_debug.js"
						: "",//去除ima3.js
					sdkLoadedCallback,
					function() {
	...
	module.prototype.showClick2PlayOverlay = function(callback, force) {
		...
		if (isInitialAd) {
			...
			.then(function() {
				startVideoCallback();//wuhard 直接跳过开始按钮进入游戏
...
/* 			self.wait(33).then(function() {
		...
		return new Promise(function(resolve, reject) {
			if (M.provider === "dfp") {
				...
				faZepto.each(rules, function(key, ruleName) {
					...
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, key, 0));
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, key, 1));
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, key, 12));
				});
	...
	module.prototype.getAdTagUrl = function(variant, priceMode, waterfallID, demandSource) {
		variant = variant || "";
		priceMode = priceMode || "";
		demandSource = demandSource || 0;
		...
		if (detection.is.iframe) {
			if (M.mcm_required) {
				...
				var matchedOrigin = 
					self.config.last_ancestor_matched ? self.config.last_ancestor_origin : (
					self.config.first_ancestor_matched ? self.config.first_ancestor_origin : (
					self.config.referrer_matched ? self.config.referrer_origin : ""));
				if (typeof matchedOrigin == "string" && 
						matchedOrigin.length > 0 && 
						!matchedOrigin.endsWith("famobi.com")) {
					descriptionUrl = matchedOrigin;	
				}
			}
		}	
		...
		M.dfp_custom_params.ds = "" + demandSource;
		...
		if (M.gam_sizes_mode > 0 &&
				M.dfp_custom_params.video &&
				M.dfp_custom_params.video === "1" &&
				demandSource == 0) {
			var creativeSizes = [{w:1920, h:1080}, {w:1280, h:720}, {w:854, h:480}, {w:728, h:480}, {w:640, h:480}, {w:640, h:360}, {w:580, h:400}, {w:480, h:360}, {w:400, h:300}, {w:336, h:280}, {w:320, h:240}, {w:300, h:250}, {w:176, h:144}, {w:1, h:1}, {w:1080, h:1920}, {w:720, h:1280}, {w:480, h:854}, {w:480, h:728}, {w:480, h:640}, {w:360, h:640}, {w:400, h:580}, {w:360, h:480}, {w:300, h:400}, {w:280, h:336}, {w:240, h:320}, {w:250, h:300}, {w:144, h:176}];

			creativeSizes = creativeSizes.filter(function(size){
					// remove sizes too big for ad request
					return size.w <= M.adWidth && size.h <= M.adHeight;
				}).map(function(size){
			...
			if (M.gam_sizes_mode == 1)
				creativeSizes = creativeSizes.slice(0, 1);
	
			szString = creativeSizes.reduce(function(str, size, index){
					return str + (index > 0 ? "|" : "") + size.w + "x" + size.h;
				}, "");
		} else if (demandSource == 1) {
			szString = "640x480|400x300";
			} else {
			// default sizes
			if (M.adWidth >= 728) {
				szString = "728x480|640x480|400x300|336x280|300x250|1x1";
		} else if (
			M.adWidth >= 640 ||
			(M.dfp_custom_params.video && M.dfp_custom_params.video === "1")
		) {
				szString = "640x480|400x300|336x280|300x250|1x1";
		} else if (M.adWidth >= 400) {
				szString = "400x300|336x280|300x250|1x1";
		} else if (M.adWidth >= 336) {
				szString = "336x280|300x250|1x1";
		} else {
				szString = "300x250|1x1";
		}
		}

		myAdTagUrl = myAdTagUrl + "&sz=" + szString;
	...
	module.prototype.init = function() {
		...
		switch (M.provider) {
			...
			case "dfp":
				...
				M.dfp_network_code = "37336410";
				if (self.config.ads.mcm_network_code && detection.is.iframe) {
					M.dfp_network_code += "," + self.config.ads.mcm_network_code;
				}
				M.dfp_ad_unit_code =
					"/" + M.dfp_network_code + "/InGameInterstitial//" + self.config.aid + "//d0";
				M.gam_sizes_mode = 1;
				M.gam_demand_source = 0;
...
fg_api.prototype.consentModule = function() {
	...
	function module() {
        ...
        this.cookieproScriptURL = '';//wuhard 去除otSDKStub.js
...
fg_api.prototype.trackingModule = function() {
	...
	module.prototype.init = function() {
		...
		window.famobi_dataLayer.push({
			...
			"firstAncestorOrigin": self.config.first_ancestor_origin || "unknown",
			"lastAncestorOrigin": self.config.last_ancestor_origin || "unknown"
		});
...
fg_api.prototype.init = function () {
	...
	var urlParams = self.getUrlParams();
	var referrerURL = urlParams.original_ref ? urlParams.original_ref : document.referrer;
	...
	if (
		...
		for (var domain of portalDomains) {
			...
			try {
				...
				if (parsedPortalURL.hostname)
					parsedPortalURLs.push(parsedPortalURL);
			} catch (error) {
	...
	function doesURLMatchPortalDomains(parsedURL) {
		var hostname = parsedURL.hostname.split(".").filter(
				function(substring) {
		...
		for (parsedPortalURL of parsedPortalURLs) {
			if (
				...
				(parsedPortalURL.hostname && parsedPortalURL.hostname.indexOf(hostname) >= 0)
			) {
	...
	if (
		...
		try {
			parsedAncestorOrigin = new URL(location.ancestorOrigins.item(location.ancestorOrigins.length - 1));
			if (parsedAncestorOrigin) {
	...
	function loadStylesheet() {
		return new Promise(function (resolve) {
			...
			newStylesheet.setAttribute(
				"href",
			 "./assets/0.2-fb5e3a79/css/play.css"//修改获取player.css路径
			);
